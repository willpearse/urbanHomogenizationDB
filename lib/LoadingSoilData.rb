...
def read_soil(file_name, city_code, sheet=nil)
  output = DataFrame.new({:date=>[],:core_id=>[],:city_parcel=>[],:date=>[],:site_no=>[],:core_no=>[],:plot_no=>[],:depth=>[],:total_L=>[],:notes=>[],:core_sections=>[],:sample_id=>[],:weight=>[],:key_no=>[],:bio_C=>[],:resp_c=>[],:NO2_NO3=>[],:NH4=>[],:bio_N=>[],:min=>[],:nit=>[],:H20=>[],:DEA=>[],:sand_percent=>[],:pH=>[]})
  curr_file = UniSheet.new file_name
  unless sheet.nil? do curr_file.set_sheet sheet end
  end
  curr_file.each do |line|
    if line[0] and line[0] != "Date"
      output << {:date=>[line[0]],:core_id=>[line[1]],:city_parcel=>[city_code+"_"+line[4].to_s],:date=>[line[3]],:site_no=>[line[5]],:core_no=>[line[6]],:plot_no=>[line[7]],:depth=>[line[8]],:total_L=>[line[9]],:notes=>[line[10]],:core_sections=>[line[11]],:sample_id=>[line[12]],:weight=>[line[13]],:key_no=>[line[14]],:bio_C=>[line[15]],:resp_c=>[line[16]],:NO2_NO3=>[line[17]],:NH4=>[line[18]],:bio_N=>[line[19]],:min=>[line[20]],:nit=>[line[21]],:H20=>[line[22]],:DEA=>[line[23]],:sand_percent=>[line[24]],:pH=>[line[25]]}
    end
  end
  return output
end
